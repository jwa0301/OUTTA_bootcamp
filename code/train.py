...
def contrastive_loss_G(fake_image, clip_model, txt_embedding, device, tau=0.5):
    ...
    image_features = clip_model.encode_image(normed_img) #(1)
    image_features = normalize(image_features)           #(2)
    
    batch_size = image_features.shape[0]
    
    sim_matrix = torch.cosine_similarity(image_features.unsqueeze(1), txt_embedding.unsqueeze(0), dim=-1)
    # image_features = image_features.unsqueeze(2).expand(batch_size,image_features.shape[1],batch_size)
    # txt_embedding = txt_embedding.unsqueeze(2).expand(batch_size,txt_embedding.shape[1],batch_size)

    # sim_matrix = cos_sim(image_features,torch.transpose(txt_embedding,0,2))
    sim_matrix /= tau
    L_cont = -tau * torch.mean(
      torch.log(
        torch.diag(
      softmax(sim_matrix)
    )))
...
def contrastive_loss_D(g_out_align, txt_embedding, tau=0.5):
    ...
    model_features = normalize(g_out_align) #(1)
    batch_size = model_features.shape[0]

    # model_features = model_features.unsqueeze(2).expand(batch_size, model_features.shape[1],batch_size)
    # txt_embedding = txt_embedding.unsqueeze(2).expand(batch_size, txt_embedding.shape[1],batch_size)
    
    # sim_matrix = cos_sim(model_features,torch.transpose(txt_embedding, 0, 2))

    sim_matrix = torch.cosine_similarity(model_features.unsqueeze(1), txt_embedding.unsqueeze(0), dim=-1)
    sim_matrix /= tau
...
def D_loss(real_image, fake_image, model_D, loss_fn, 
    ...
    if use_uncond_loss:
        ...
        predict_fake, _ = model_D(fake_image)
        loss_d_comp['d_loss_fake_uncond'] = loss_fn(predict_fake.squeeze(), d_fake_label).mean()
        

        predict_real, _ = model_D(real_image)
        loss_d_comp['d_loss_real_uncond'] = loss_fn(predict_real.squeeze(), d_real_label).mean()


    predict_cond_fake, d_out_align_fake = model_D(fake_image, mu)
    loss_d_comp['d_loss_fake_cond'] = loss_fn(predict_cond_fake.squeeze(), d_fake_label).mean()


    predict_cond_real, d_out_align_real = model_D(real_image, mu)
    loss_d_comp['d_loss_real_cond'] = loss_fn(predict_cond_real.squeeze(), d_real_label).mean()
...
def G_loss(fake_image, model_D, loss_fn,
    ...
    if use_uncond_loss:
        ...
        predict, _ = model_D(fake_image)
        loss_g_comp['g_loss_uncond'] = loss_fn(predict.squeeze(), g_label).mean()


    predict_cond, g_out_align = model_D(fake_image, mu)
    loss_g_comp['g_loss_cond'] = loss_fn(predict_cond.squeeze(), g_label).mean()
...
def train_step(train_loader, noise_dim, device, model_G, model_D_lst, optim_g, optim_d_lst, 
    ...
    for iter, batch in enumerate(train_loader):
        ...
        # d_fake_label = torch.zeros(BATCH_SIZE, 1, 1, 1, dtype = torch.float32, device = device)
        # d_real_label = torch.ones(BATCH_SIZE, 1, 1, 1, dtype = torch.float32, device = device)
        # g_label = torch.ones(BATCH_SIZE, 1, 1, 1, dtype=torch.float32).to(device)
